...
public class OpeningRepository(AppDbContext db) : IOpeningRepository
{
    private IQueryable<Opening> WithIncludes() =>
        db.Openings
            .Include(o => o.AnimeEntry).ThenInclude(e => e.Anime)
            .Include(o => o.Artist);

    public async Task<IEnumerable<Opening>> GetAllAsync(CancellationToken ct = default) =>
        await WithIncludes().OrderBy(o => o.AnimeEntry.Anime.Title).ThenBy(o => o.AnimeEntry.Title).ThenBy(o => o.OrderNumber).ToListAsync(ct);
        ...
        string? filterText, string? sorting, int skipCount, int maxResultCount, CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filterText))
            query = query.Where(o => o.SongTitle.Contains(filterText)
                || o.AnimeEntry.Title.Contains(filterText)
                || o.AnimeEntry.Anime.Title.Contains(filterText));
        ...
        query = sorting?.ToLowerInvariant() switch
        {
            ...
            "animetitle desc"     => query.OrderByDescending(o => o.AnimeEntry.Anime.Title).ThenBy(o => o.OrderNumber),
            "ordernumber"         => query.OrderBy(o => o.OrderNumber),
            "ordernumber desc"    => query.OrderByDescending(o => o.OrderNumber),
            _                     => query.OrderBy(o => o.AnimeEntry.Anime.Title).ThenBy(o => o.AnimeEntry.Title).ThenBy(o => o.OrderNumber)
        };
    ...
    public async Task<(IEnumerable<Opening> Items, int TotalCount)> GetPagedByAnimeEntryIdAsync(
        Guid animeEntryId, string? filterText, string? sorting, int skipCount, int maxResultCount, CancellationToken ct = default)
    {
        var query = WithIncludes().Where(o => o.AnimeEntryId == animeEntryId);
        ...
        CancellationToken ct = default)
    {
        var query = WithIncludes()
            .Include(o => o.AnimeEntry).ThenInclude(e => e.Anime).ThenInclude(a => a.AnimeTags)
            .AsQueryable();
        ...
        if (tagIds?.Any() == true)
            query = query.Where(o => o.AnimeEntry.Anime.AnimeTags.Any(at => tagIds.Contains(at.TagId)));
